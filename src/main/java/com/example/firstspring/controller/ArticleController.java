...
public class ArticleController {
    ...
    @GetMapping("/articles/{id}/delete") //html에서는 Delete를 지원하지 않아서 GetMapping으로 구현
    public String delete(@PathVariable Long id, RedirectAttributes rttr){ //URL 경로를 통해 받은 id값을 가져온다.
        // 1 : 삭제 대상을 가리킨다 ! (삭제 대상을 가져온다.)
        Article target = articleRepository.findById(id).orElse(null);
        log.info(target.toString());

        // 2 : 대상을 삭제한다.
        if (target != null){
            articleRepository.delete(target); // 삭제 대상 : target
